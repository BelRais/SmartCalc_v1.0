...
START_TEST(test_calc_1) {
  ...
  double result;
  int error = 0;
  result = s21_main_calculator(input, 0.0, &error);
...
START_TEST(test_calc_10) {
  ...
  double result;
  int error = 0;
  result = s21_main_calculator(input, x, &error);
...
START_TEST(test_calc_23) {
  char input[255] = "5/0";
  int error = 0;
  s21_main_calculator(input, 0.0, &error);
...
START_TEST(test_calc_credit_1_annu) {
  double loan_amount = 100000;
  int term = 12;
  double interest_rate = 10;
  int type = 1;
  char monthly_payment_true[256] = "8791.59";
  double overpay_true = 5499.06;
  double total_pay_true = 105499.06;
  char monthly_payment_model[256] = "";
  double overpay_model = 0;
  double total_pay_model = 0;
  double result = 0;
  calculate_credit(loan_amount, term, interest_rate, type,
                   monthly_payment_model, &overpay_model, &total_pay_model);
  if (strcmp(monthly_payment_true, monthly_payment_model) == 0 &&
      overpay_true == overpay_model && total_pay_true == total_pay_model) {
    result = 1;
  }
  ck_assert_double_eq(result, 1);
}
...
START_TEST(test_calc_credit_1_dif) {
  double loan_amount = 100000;
  int term = 12;
  double interest_rate = 10;
  int type = 2;
  char monthly_payment_true[256] = "9166.67 ... 8402.78";
  double overpay_true = 5416.67;
  double total_pay_true = 105416.67;
  char monthly_payment_model[256] = "";
  double overpay_model = 0;
  double total_pay_model = 0;
  double result = 0;
...
int main() {
  Suite *suite = suite_create("s21_main_calculator: ");
  TCase *tcase_core = tcase_create("s21_main_calculator: ");
  ...
  tcase_add_test(tcase_core, test_calc_credit_1_annu);

  tcase_add_test(tcase_core, test_calc_credit_1_dif);
